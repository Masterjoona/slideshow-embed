...
func MakeVideo(collagePath string, inputDir string, outputPath string) error {
	...
	fmt.Println("ffmpeg", "-loop", "1", "-framerate", "1", "-i", collagePath, "-i", audioFilePath, "-map", "0", "-map", "1:a", "-c:v", "libx264", "-preset", "ultrafast", "-tune", "stillimage", "-vf", "fps=10,format=yuv420p", "-c:a", "copy", "-shortest", output)
